...
def data_preprocessing(img, mask, num_class):
    ...
    number_of_images, height, width, channles= mask.shape                           
    mask_reshape = mask.reshape(-1,1)                                               
    encoded_mask = labelencoder.fit_transform(mask_reshape.ravel())                
    original_encoded_mask = encoded_mask.reshape(number_of_images, height, width )   
    mask = np.expand_dims(original_encoded_mask, axis = 3)                         
...
train_img_gen = TFDataLoader(train_img_path, train_mask_path, num_class=9)         
